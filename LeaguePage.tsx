...
import SubdivisionDropdown from "./SubdivisionDropdown"
...
export default function LeaguePage({ leagueName }: LeaguePageProps) {
  ...
  const [selectedSubdivision, setSelectedSubdivision] = useState<string>("FBS")
  const [selectedConference, setSelectedConference] = useState<string>("")
  const [initialPlayers] = useState<Player[]>(() => {
  ...
  const watchlistedPlayers = useMemo(() => {
    return players.filter((player) => player.listTypes.includes("Watched"))
  }, [players])

  const treatedPlayers = useMemo(() => {
    return players.filter((player) => player.listTypes.includes("Treated"))
  }, [players])

  const priorityPlayers = useMemo(() => {
    return players.filter((player) => player.listTypes.includes("Priority"))
  }, [players])
    ...
    setPlayers(
      players.map((player) => {
        if (player.name === playerName) {
          const updatedListTypes = player.listTypes.includes(newListType)
            ? player.listTypes.filter((type) => type !== newListType)
            : [...player.listTypes, newListType]

          const primaryListType = updatedListTypes.length > 0 ? updatedListTypes[0] : "None"

          return {
            ...player,
            listTypes: updatedListTypes,
            listType: primaryListType,
            isWatchlisted: updatedListTypes.includes("Watched"),
            isTreated: updatedListTypes.includes("Treated"),
            isPriority: updatedListTypes.includes("Priority"),
          }
        }
        return player
      }),
  ...
  const handleSubdivisionSelect = (subdivision: string, conference: string, team: string) => {
    setSelectedSubdivision(subdivision)
    setSelectedConference(conference)
    setSelectedTeam(team)
    // Here you would typically fetch subdivision, conference, and team-specific data
    if (team) {
      const filteredPlayers = initialPlayers.filter((player) => player.team === team)
      setPlayers(filteredPlayers)
      setViewAllPlayers(false)
    } else if (conference) {
      // For basketball and hockey, filter by conference
      const filteredPlayers = initialPlayers.filter((player) => player.conference === conference)
      setPlayers(filteredPlayers)
      setViewAllPlayers(false)
    } else {
      setPlayers(initialPlayers)
  ...
  const renderToolbar = () => (
    <div className="flex justify-between mb-4">
      <div className="flex space-x-2">
        ...
        {["NBA", "NHL", "NFL", "MLB"].includes(leagueName.toUpperCase()) ? (
          <TeamDropdown league={leagueName} onTeamSelect={handleTeamSelect} />
        ) : (
          (leagueName.toUpperCase() === "FOOTBALL" ||
            leagueName.toUpperCase() === "BASKETBALL" ||
            leagueName.toUpperCase() === "HOCKEY" ||
            leagueName.toUpperCase() === "BASEBALL") && (
            <SubdivisionDropdown
              onSubdivisionSelect={handleSubdivisionSelect}
              sport={leagueName.toLowerCase() as "football" | "basketball" | "hockey" | "baseball"}
            />
          )
        )}
        ...
        <Select onValueChange={(value) => handleSort(value as SortKey)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="contractLength">ContractLength</SelectItem>
                  ...
                  <tr key={index} className="hover:bg-gray-100">
                    ...
                    <td className="border p-2">
                      <Button
                        variant="link"
                        className={`
                          ${player.listTypes.includes("Priority") ? "text-red-600 hover:text-red-800" : ""}
                          ${player.listTypes.includes("Treated") ? "text-purple-600 hover:text-purple-800" : ""}
                          ${player.listTypes.includes("Watched") ? "text-yellow-600 hover:text-yellow-800" : ""}
                          ${player.listTypes.includes("None") ? "text-black hover:text-gray-800" : ""}
                          ${!["Priority", "Treated", "Watched", "None"].some((type) => player.listTypes.includes(type)) ? "text-blue-600 hover:text-blue-800" : ""}
                        `}
                    ...
                    <td className="border p-2">
                      {listTypes.map((type) => (
                        <div key={type} className="flex items-center space-x-2">
                          <Checkbox
                            id={`${player.name}-${type}`}
                            checked={player.listTypes.includes(type)}
                            onCheckedChange={(checked) => {
                              handleListTypeChange(player.name, type as ListType)
                            }}
                          />
                          <label
                            htmlFor={`${player.name}-${type}`}
